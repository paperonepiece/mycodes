##Generate random N node vacancy defects in ZIF & Not complete
...
Ndrop = 17      # lines above atom position informations 16/17 for ZIF8/4
totalnum = 2176 # total number of all types of atoms
abovenum = 1536 # the atoms which ordinal number smaller than defect atoms
Nnum    = 512   # total number of defect atoms before get deleted
belownum = totalnum-abovenum-Nnum  # atoms with ordinal number bigger than defect atoms
lendig = 4
Zndis = float(2.5) # Angstorm
...
delnum = round(Nnum*eval(input("Please input the concentration of Zn node defect you need：")))
newnum = totalnum - delnum # total number of all types of atoms after defect generation
nodef = open("ZIF4.dat", "r+") 
defec = open("0.15ZIF4.dat", "w")

#  generate defect nodes with wanted distance for preventing ligand missing 
while(len(dellist)<delnum):
    break_flag = False
    nodef.seek(0,0)
    x = random.randint(1,Nnum)
    if x not in dellist:
...
linenum = 0
linechange = 0
for k in range(abovenum+1,abovenum+Nnum+1):
    linenum += 1
    line = nodef.readline()
...
print('Zn node defects generation has done, '+
      'defect concentration is '+str(format(delnum/Nnum*100,'.0f'))+'%')
