...
with open('K_total_python.dat','w') as fidw,\
    ...
    fidw.write('Timestep  KV  xyz  KK  xyz  KC  K  xyz  StdK'+'\n')
    fidv.write('Timestep  KV  xyz  StdKV'+'\n')
    for m in range(1,int(dump_t+1)):     
        fidw.write(format(m*interval*dt*1e12,'<10.5f'))
        fidw.write(format(AveKV[(m-1)*interval],'^12.6f')+' '+
                   format(AveKVx[(m-1)*interval],'^12.6f')+' '+
                   format(AveKVy[(m-1)*interval],'^12.6f')+' '+
                   format(AveKVz[(m-1)*interval],'^12.6f')+' '+
                   format(AveKK[(m-1)*interval],'^12.6f')+' '+
                   format(AveKKx[(m-1)*interval],'^12.6f')+' '+
                   format(AveKKy[(m-1)*interval],'^12.6f')+' '+
                   format(AveKKz[(m-1)*interval],'^12.6f')+' '+
